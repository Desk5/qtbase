...
void QWindowsWindow::updateTransientParent() const
{
    ...
    if (const QWindow *tp = window()->transientParent()) {
        if (const auto *tw = QWindowsBaseWindow::baseWindowOf(tp)) {
            if (tw->isForeignWindow() || !static_cast<const QWindowsWindow*>(tw)->testFlag(WithinDestroy)) // Prevent destruction by parent window (QTBUG-35499, QTBUG-36666)
